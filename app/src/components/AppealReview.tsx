...
export default function AppealReview() {
  const { appealDraft, grievances, lang, startAppealDraft, sendAppeal, setView } = useAppStore();
  const d = dict(lang);
    ...
    return (
      <Box sx={{ p: 4 }}>
        ...
        <Typography className="longform" variant="body2" color="text.secondary" sx={{ mt: 0.5, maxWidth: 560, lineHeight: 1.6 }}>
          {d.review.appealEmpty1}<strong>{d.review.appealEmptyPoor}</strong>.{d.review.appealEmpty2}
          {eligible.length > 0 ? d.review.appealEligible : d.review.appealAsk}
        </Typography>
      </Box>
    );
  }
  ...
  const confirm = () => {
    if (window.confirm(d.review.sendConfirm)) {
      sendAppeal();
      setView("case");
    }
  };
    ...
    <Box sx={{ maxWidth: 820, mx: "auto", width: 1, px: { xs: 1.5, md: 2 }, py: 2.5 }}>
      <Paper elevation={0} sx={{ overflow: "hidden", borderRadius: "12px" }}>
        <PageHeader
          ...
          sub={d.review.appealSub(ministryOf(g.ministryId)?.appellateAuthority ?? "Nodal Appellate Authority")}
        />
        <Stack spacing={2.5} sx={{ p: { xs: 2, md: 3 } }}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "140px 1fr" }, gap: { xs: 0.5, sm: 2 } }}>
            <Typography variant="overline" sx={{ color: "text.secondary", fontSize: "0.6875rem", pt: 0.5 }}>{d.review.originalRelief}</Typography>
            <Typography className="longform" variant="body2" sx={{ lineHeight: 1.7 }}>{g.reliefRequested}</Typography>
            <Typography variant="overline" sx={{ color: "text.secondary", fontSize: "0.6875rem", pt: 0.5 }}>{d.review.disposalAppealed}</Typography>
          ...
          <TextField
            ...
            helperText={d.review.argumentHelper}
          />

          <Divider />
          <Stack direction="row" spacing={1.5} sx={{ justifyContent: "flex-end" }}>
            <Button color="inherit" onClick={() => setView("case")}>{d.review.goBack}</Button>
            <Button variant="contained" color="secondary" disabled={!valid} startIcon={<Gavel />} onClick={confirm}>
              {d.review.confirmAppeal}
            </Button>
          </Stack>
        </Stack>
      </Paper>
