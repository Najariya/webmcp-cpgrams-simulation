...
The full lifecycle works through the agent, end to end, safely: gated filing with memorable IDs, the hero SLA journey, Poor-rating → live tool-registry change → evidence-grounded appeal. 28 unit tests + a 20-prompt eval suite + adversarial tests, all green. A premium government-grade UI that still reads honestly as a simulation.
